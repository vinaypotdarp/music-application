...
const REQUEST_TIMEOUT_MS = 9000; // stay under Vercel's ~10s default serverless function limit rather than hanging
...
export async function chatDecision({ message, history = [], systemContext, allowedMoodTags }) {
  ...
  const body = {
    ...
    generationConfig: {
      ...
      temperature: 0.9,
      // gemini-3.5-flash "thinks" before answering by default, which can push
      // latency past our timeout (and Vercel's own function time limit) for a
      // simple JSON decision like this. Keep it fast and cheap.
